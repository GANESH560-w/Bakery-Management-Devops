...
let orders = [];
let otps = {}; // Simple in-memory OTP storage
    ...
    req.on('end', () => {
        ...
        try {
            ...
            // Auth
                ...
                res.end(JSON.stringify({status: 'ok', message: 'OTP sent successfully'}));
            }
            else if (url === '/api/verify-otp' && method === 'POST') {
                const { email, otp } = JSON.parse(body);
                if (otps[email] && otps[email] === otp) {
                    delete otps[email];
                    res.end(JSON.stringify({status: 'ok', message: 'OTP verified'}));
                } else {
